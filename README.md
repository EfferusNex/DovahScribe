...
Комплекс решает ключевые проблемы машинного и любительского перевода:
* ❌ Реплики переведены изолированно вслепую, теряя нить разговора и эмоциональный тон.
* ❌ Девушки-спутницы и персонажи говорят о себе в мужском роде (*«Я нашёл»*, *«Я готов»*).
* ❌ Ложные друзья переводчика превращают *«Staff»* в *«Персонал»*, *«Race»* в *«Гонку»*, а *«Chest»* в *«Грудь»*.
* ❌ Ломаются игровые плейсхолдеры (`<ALIAS=Player>`, `%s`, `%d`), приводя к вылетам движка.
* ❌ Оверрайды модов сбрасывают переведённые имена на английский или замусоривают скрипты.

**DovahScribe** полностью устраняет эти проблемы благодаря прямому бинарному парсингу, контекстному графу диалогов, анализу байткода Papyrus-скриптов, каноничной базе 1С и строгому 5-ступенчатому аудиту **QualityGate**.
...
### 🏛️ 4. Каноничная база 1С и защита от ложных друзей (Anti-Trap)
...
│   └── false_friends.json      # Реестр ложных друзей переводчика
